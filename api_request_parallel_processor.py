...
from utils import format_prompt, format_user_chat
...
async def process_api_requests_from_file(
    ...
    temperature: float,
    max_tokens: int,
    completions_generate: int,
    proxy_url: str, # add to visit openai in China
    api_key: str,
    ...
    if os.path.isfile(config['selector_desc']):
        with open(config['selector_desc'], "r") as file:
            selector_desc = ''.join(file.readlines())
    else:
        selector_desc = config['selector_desc']
                ...
                if not queue_of_requests_to_retry.empty():
                    ...
                    try:
                        ...
                        if 'chat' in request_url:
                            request_json['messages'] = [{"role": "user", "content": format_user_chat(request_json['demo_examples'], request_json['question'])}]
                        else:
                            request_json['prompt'] = format_prompt(selector_desc, request_json['demo_examples'],
                                                               request_json['question'])
                        request_json['model'] = request_model
                        request_json['temperature'] = temperature
                        request_json['max_tokens'] = max_tokens
                        request_json['n'] = completions_generate
                        del request_json['demo_examples']
                        del request_json['question']
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    url=request_url, headers=request_header, json=self.request_json, proxy=proxy_url
                ) as response:
...
if __name__ == "__main__":
    ...
    if 'chat' in config['request_url']:
        args.max_tokens_per_minute = int(90000 * 0.5)
        args.max_requests_per_minute = int(3500 * 0.5)
    if '16k' in config['model']:
        args.max_tokens_per_minute = int(180000 * 0.2)
        args.max_requests_per_minute = int(3500 * 0.2)
    # run script
    asyncio.run(
        process_api_requests_from_file(
            requests_filepath=config['query_file'],
            save_filepath=config['gpt_answer_file'],
            request_url=config['request_url'],
            request_model=config['model'],
            temperature=config['temperature'],
            max_tokens=config['max_tokens'],
            completions_generate=config['completions_generate'],
